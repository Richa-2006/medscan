...
app.get("/api/auth/google/callback",
  ...
  async (req, res) => {
    ...
    res.redirect(`${process.env.FRONTEND_URL}/auth.html?token=${token}&name=${encodeURIComponent(req.user.name)}&email=${encodeURIComponent(req.user.email)}`);
  }
);

app.get("/api/auth/failed", (req, res) => {
  res.redirect(`${process.env.FRONTEND_URL}/auth.html?error=Google login failed`);
});
